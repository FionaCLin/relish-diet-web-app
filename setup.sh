...
sudo su - postgres -c "psql postgres -U $USER -tAc \"SELECT 1 FROM pg_roles WHERE rolname='dev'\"" |
grep -q 1 || sudo -u postgres createuser -D dev
sudo su - postgres -c "psql postgres -U $USER -c \"ALTER USER dev WITH PASSWORD 'dev';\"" > /dev/null
