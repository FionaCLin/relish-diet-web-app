...
class RecipePage extends React.Component {
  constructor(props) {
    ...
    let bookmarked = !isUndefined(
      this.props.users
        .find((x) => x.id === this.props.curr_user)
        .bookmarks.find((y) => y === recipe.id)
    )
      ? true
      : false;
  ...
  fillStar = (number, clickable) => {
    ...
    for (let i = 0; i < 5; i++) {
      let starType =
        i < number
          ? "glyphicon glyphicon-star green_star"
              ...
              <Col className="text-black-50 text-capitalize">
                ...
                {this.props.users && (
                  <h6>
                    by{" "}
                    {
                      this.props.users.find((x) => x.id == recipe.creator)
                        .username
                    }
                  </h6>
                )}
              ...
              <Col sm={12} md={7} lg={8} xl={8}>
                <div className="panel panel-default">
                  ...
                  >
                    <tbody>
                      <tr>
                        {constants.mealPlanner.macroNutrients.map(
                          (nutrient) => {
                            let measure =
