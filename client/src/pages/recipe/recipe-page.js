...
//   render() {
const RecipePage = ({recipe, canBookmarked, loading}) => {
  return (
      ...
      {!loading && (
        <Card>
          <Card.Header className='justify-content-around py-0 '>
            <Row className='justify-content-center align-items-center'>
              <Col className='text-black-50 text-capitalize'>
                <h4>{recipe.title}</h4>&nbsp; <h6>by {`${recipe.author.firstName} ${recipe.author.lastName}`}</h6>
              </Col>
              {canBookmarked && (
                <Col>
                  <button
                    // className={`text-black-50 text-capitalize btn btn${
                    //   bookmarked ? '-success' : '-default'
                    // } bookmarkBtn`}
                    className={`btn btn-default bookmarkBtn`}
                    type='button'
          ...
          <Card.Body className='px-4'>
            <Row>
              <Col sm={12} md={6} lg={6} xl={6} className='panel panel-default'>
                <h4 className='text-black-50 text-capitalize'> Nutrient</h4>
                  ...
                  {constants.mealPlanner.macroNutrients.map((nutrient) => (
                    <li>
                      <p>
                        ...
                        {recipe[nutrient.toLocaleLowerCase()]}
                      </p>
                    </li>
                  ))}
                </ul>
              ...
              <Col className='panel panel-default'>
                <h4 className='text-black-50 text-capitalize'> Ingredients</h4>
                <ul className='macro-list'>
                  {recipe.ingredients.map((item) => (
                    <li key={item.id}>
                      <p>
                        {item.RecipeIngredient.amount.toFixed(2)} {item.UOM} <span className='name'> {item.name}</span>
                      </p>
                    </li>
                  ))}
                </ul>
            ...
            <Row className='panel panel-default'>
              <h4 className='text-black-50 text-capitalize'> Method</h4>
              <Col>{recipe.method}</Col>
                    ...
                    {recipe.images.map((item, index) => {
                      ...
                      return (
                        <div className={index === 0 ? 'item active' : 'item'}>
                          <img src={`../images/${item.url}`} style={{width: '100%', height: '100%'}} alt='img'></img>
                        </div>
                      );
                    })}
                  </div>
