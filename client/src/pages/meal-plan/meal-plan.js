...
class MealPlanner extends React.Component {
  constructor(props) {
    ...
    //     : mealPlans.find((x) => x.id == this.props.match.params.id);
  ...
  //     let bookmarks = users.find((x) => x.id === curr_user).bookmarks;
  //     recipeInfo.forEach((recipe) => {
  ...
  //     if (this.props.match.params.mode !== mealPlanner.ADD_MEAL_PLANNER) {
      ...
      <Container className='pt-2 m-auto'>
        <div class='form-group mealPlannerForm'>
          <div class='mealPlanHeader'>
            <div>
              <h4>{this.props.match.params.mode === mealPlanner.ADD_MEAL_PLANNER ? 'New' : 'Edit'} Meal Plan</h4>
              <input
                type='text'
                class='form-control mealPlannerTitle'
          ...
          {/* <div
            class='panel panel-default overallPlannerMacros'
            style={{
              marginTop: this.props.match.params.mode === mealPlanner.EDIT_MEAL_PLANNER ? '40px' : '10px',
              marginRight: this.props.match.params.mode === mealPlanner.VIEW_MEAL_PLANNER ? '0px' : '',
            }}
          >
            <table class='table table-sm table-bordered table-striped' style={{textAlign: 'center'}}>
              <tbody>
                ...
                {this.props.match.params.mode === mealPlanner.ADD_MEAL_PLANNER ? (
                  <tr class='overallMacroRow'>
                    <td class='overallMacroRow' style={{textAlign: 'left'}}>
                      Goal
                    </td>
                    {goal.map((item) => {
                ...
                <tr class='overallMacroRow'>
                  ...
                  {mealPlanner.macroNutrients.map((nutrient) => {
        ...
        {/* <div class='macroLeft'>
          <table class='table table-bordered'>
            <thead>
              <tr>
                {mealPlanner.daysOfWeek.map((item) => {
                  return (
                    <th scope='col' class='mealPlannerDays'>
                      {item}
                    </th>
                  );
                })}
                ...
                let timeSlots = dailyMeals.map((day, dayKey) => {
                  if (!isNull(day[mealKey])) {
                    if (!isUndefined(this.getRecipe(day[mealKey]))) {
                      return (
                        <td
                          style={bg_img(this.getRecipe(day[mealKey]).img[0])}
                          class='planner_img'
                          draggable={this.props.match.params.mode !== mealPlanner.VIEW_MEAL_PLANNER ? 'true' : 'false'}
                          onClick={
                            this.props.match.params.mode === mealPlanner.VIEW_MEAL_PLANNER
                          ...
                          onDrop={(e) => this.onDrop(e, [dayKey, mealKey])}
                        >
                ...
                let macroHead =
                  nutrient === 'Energy' ? (
                    <th scope='row' rowspan='5'>
                      <div class='vertical macroDiv'>MACROS</div>
                    </th>
                  ) : null;
                  ...
                  return (
                    <td class='macro_img'>
                      ...
                      <div class='macroRight'>
                        {this.calculateNutrient(day, nutrient)} {this.getMeasurement(nutrient)}
                      </div>
                    </td>
                  );
                });
          ...
          <div style={{float: 'right'}}>
            <div style={{float: 'left'}} class='dropArea'>
                    ...
                    {recipeList.map((recipe) => {
                        ...
                        return (
                          <tr>
                            ...
                            <td style={{float: 'left', width: 75}}>
                              {recipe.macros[nutrient]} {this.getMeasurement(nutrient)}
                            </td>
                          </tr>
                        );
                      });
                      return (
                        <div class='list-group-item list-group-item-action planner_img_wrapper'>
                          <div
                            class='macroInfo btn-xs btn-default'
                            onMouseEnter={(e) => this.macroOver(e, recipe.id)}
                            onMouseLeave={(e) => this.macroOut(e)}
                          >
                          ...
                          <div
                            ...
                            onDragStart={(e) => this.onDragStart(e, recipe.id)}
                          ></div>
                          <div class='overlay'>
              ...
              <div class='macroRight'>
                <button onClick={(e) => this.editPlan(e)} class='btn btn-success editSubmit'>
                  <Link to='/meallist'>
                  {this.props.match.params.mode === mealPlanner.EDIT_MEAL_PLANNER ? 'Edit' : 'Create'}
                  </Link>
                </button>
                <Link to='/meallist'>
                <button class='btn btn-secondary editCancel'>Cancel</button>
                </Link>
              </div>
            </div>
            <div style={{float: 'left', width: 20}}>
              {mealPlanner.recipeList.map((item) => {
                if (
                  this.props.match.params.mode == mealPlanner.EDIT_MEAL_PLANNER &&
                  item.recipes === mealPlanner.RECOMMENDED
                ) {
                  return null;
                } else {
                  return (
                    <button
                      class={currRecipes === item.recipes ? 'btn sideButton btn-success' : 'btn sideButton btn-default'}
                      title={item.title}
                      onClick={(e) => this.changeRecipe(e, item.recipes)}
                    >
              ...
              <div class='panel-body pre-scrollable list-group' style={{height: '651px'}}>
                <div class='list-group'>
                  {!isNull(this.state.currRecipe) ? (
                    ...
                    <div>Click on a recipe to view</div>
                  )}
                </div>
              </div>
            </div>
